...
from src.services.model_service import evaluate_all_models, prepare_training_data  # noqa: E402
...
def run_split_strategy_ablation(df, out_dir: Path) -> dict:
    ...
    eval_default = evaluate_all_models({"xgboost": model_default}, prepared_default)["xgboost"]
    ...
    eval_holdout = evaluate_all_models({"xgboost": model_holdout}, prepared_holdout)["xgboost"]
    ...
    print(f"{'Split':40s} {'F1':>6s} {'PR-AUC':>7s} {'ROC-AUC':>8s} {'Test n':>7s} {'Test pos-rate':>13s}")
...
def run_imbalance_strategy_ablation(df, out_dir: Path) -> dict:
    ...
    print(f"{'Strategy':52s} {'Precision':>10s} {'Recall':>8s} {'F1':>6s} {'PR-AUC':>7s}")
...
def main() -> None:
    ...
    out_dir = PATHS["reports_experiments"] / f"{dataset_name}_{version}_advanced_ablation"
